...
class AddWatchlist(APIView):
    ...
    def post(self, request):
        ...
        if not tickersymbol:
            return Response(
                {"message": "Missing Ticker"}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        watchlist_entry, created = Watchlist.objects.get_or_create(
            user=request.user, stock=stock_entry
        )
        ...
        return Response(
            {"message": "Ticker already in watchlist"}, status=status.HTTP_200_OK
        )
...
class RemoveWatchlist(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"message": "Stock Not In Watchlist"}, status=status.HTTP_404_NOT_FOUND
        )
...
class FetchWatchlist(APIView):
    ...
    def get(self, request):
        ...
        watchlist_entries = Watchlist.objects.filter(user=user).order_by(
            "stock__ticker"
        )
...
class FetchAIRecommendations(APIView):
    ...
    def get(self, request):
        in_watchlist = Watchlist.objects.filter(user=request.user).values_list(
            "stock_id", flat=True
        )

        recommendations = (
            Stock.objects.exclude(id__in=in_watchlist)
            .filter(
            ...
            .order_by("-expected_percentage_change_in_price")[:5]
        )
