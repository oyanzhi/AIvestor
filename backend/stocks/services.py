...
def update_all_stock_metrics(verbose=False):
    ...
    for stock in stocks:
        try:
            ...
            stock.revenue_growth = info.get('revenueGrowth') or info.get('revenueQuarterlyGrowth')
            ...
            if income_stmt is not None:
                if 'Interest Expense' in income_stmt.index:
                    interest_expense = income_stmt.loc['Interest Expense', income_stmt.columns[0]]
            ...
            if 'Capital Expenditure' in cashflow.index:
                ...
                stock.capital_expenditures = Decimal(str(latest_capex)) if pd.notna(latest_capex) else None
            else:
