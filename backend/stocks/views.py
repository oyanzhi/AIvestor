...
class UpdateStocksView(APIView):
    ...
    def post(self, request):
        ...
        return Response(
            {"message": "Stock metrics updated."}, status=status.HTTP_200_OK
        )
    ...
    def get(self, request):
        ...
        try:
            ...
            history_data = [
                {
                    "date": date.strftime(
                        "%Y-%m-%d %H:%M" if "m" in interval else "%Y-%m-%d"
                    ),
                    "close": (
                        round(row["Close"], 2) if not pd.isna(row["Close"]) else None
                    ),
                }
                for date, row in hist.iterrows()
            ]
            return Response(
                {"symbol": symbol, "historyData": history_data},
