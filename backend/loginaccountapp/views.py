...
class LoginAccountAppView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            ...
            response = Response(
                {"message": "Login Successful", "access": access_token},
...
class GoogleSignInView(APIView):
    ...
    def post(self, request):
        ...
        if not token:
            return Response(
                {"Error": "ID token missing."}, status=status.HTTP_400_BAD_REQUEST
            )
        ...
        try:
            user = AccountDatabase.objects.get(
                oauth_provider="google", oauth_provider_id=sub
            )
...
class TokenRefreshView(APIView):  # currently setup but not being used
    ...
    def post(self, request):
        ...
        try:
            ...
            return Response(
                {"Error": "Refresh Token Cannot be Generated"},
                status=status.HTTP_401_UNAUTHORIZED,
            )
